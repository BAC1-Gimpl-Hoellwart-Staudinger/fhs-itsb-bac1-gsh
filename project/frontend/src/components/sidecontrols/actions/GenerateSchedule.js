...
function GenerateSchedule() {
  const { LOCAL_STORAGE_DATASETS_KEY, datasetSize, setStats } =
  ...
  function getSelectedDataset() {
    const localStorageDatasetItems = window.localStorage.getItem(
  ...
  useEffect(() => {
    ...
    if (!localStorageDatasetItems || localStorageDatasetItems.length === 0)
      return;
  ...
  return (
    <ActionFormContainer onSubmit={handleSubmit}>
      ...
      <ActionButton type="button" variant={0} onClick={handleClickViewDataset}>
        View Dataset
      </ActionButton>
